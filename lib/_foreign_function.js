...
function ForeignFunction(cif, funcPtr, returnType, argTypes) {
  ...
  const resultSize =
    returnType.size >= ref.sizeof.long ? returnType.size : FFI_ARG_SIZE;
  assert(resultSize > 0);
  ...
  const proxy = function () {
    ...
    if (arguments.length !== numArgs) {
      throw new TypeError(
        "Expected " + numArgs + " arguments, got " + arguments.length
      );
    }
  ...
  proxy.async = function () {
    ...
    if (argc !== numArgs + 1) {
      throw new TypeError(
        "Expected " + (numArgs + 1) + " arguments, got " + argc
      );
    }
    ...
    if (typeof callback !== "function") {
      throw new TypeError(
        "Expected a callback function as argument number: " + (argc - 1)
      );
    }
  ...
  proxy.promise = function () {
    debug("invoking promise proxy function");
